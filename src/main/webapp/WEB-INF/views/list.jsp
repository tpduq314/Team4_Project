...
<body>
		...
		<c:choose>
				...
				<c:forEach var="p" items="${plist }">
					<div class="res_area">
						<fieldset style="border: 0px">
							<div class="user_name">
								<a href="/Project/login">로그인 후 이용해주세요!</a>
							</div>
							<div class="res_name">${p.pack_name}카테고리를선택하셨어요!</div>
			...
			<c:otherwise>
				<c:forEach var="p" items="${plist }">
					<div class="res_area">
						<fieldset style="border: 0px">
							<div class="user_name">${id}님안녕하세요!</div>
							<div class="res_name">${p.pack_name}카테고리를선택하셨어요!</div>
							<div class="res_start">출발일은 ${p.pack_start_date }입니다.</div>
						</fieldset>
					</div>
	...
	<section class="serv_list">
		<div class="bList_count"
			style="margin-left: 400px; margin-bottom: 20px; font-weight: bold; font-size: 20px;">검색
			결과:${totalCount}개</div>
		<div class="container">
					...
					<c:forEach var="p" items="${plist}">
						<div class="box">
							...
							<div class="text">
								...
								<p class="a_price" id="pack_price01" 
									style="color: red; font-size: 16px;" >${p.pack_price01 }원~</p>
									
									
								<button
									onclick="location='travel_description?main_category=${p.pack_main_cate}&sub_category=${p.pack_sub_cate}&pack_code=${p.pack_code}';">예약하기</button>
							</div>
						</div>
					</c:forEach>
				</c:if>
