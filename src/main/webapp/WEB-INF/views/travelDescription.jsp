		...
		<form name="res" method="post" action="res">
			<div id="des_top">
				...
				<div id="product_code">
					...
					<div>
						<input value="${p.pack_code}" id="pack_code" name="pack_code"
							readonly="readonly">
					</div>
				</div>
			</div>
			...
			<div id="main_description">
				...
				<div class="main_schedule" style="height: 270px">
					...
					<div id="price_letter">
						...
						추가요금 발생됩니다. <br />-1인 객실 사용료: 400,000원 <br />-여행 기간 중 만 2세가 되는
						경우 요금 별도 문의
					</div>
				</div>
				...
				<div class="main_schedule" style="height: 1500px;" id="div3">
					...
					<!-- 호텔 정보 -->
					<h2 style="font-size: 16px; margin: 20px 0px 0px 10px;">01
						호텔정보</h2>
						...
						<c:if test="${!empty hlist}">
							...
							<div class="hi_top">
								<c:forEach var="h" items="${hlist}">
									<input type="button" class="btn_class"
										onclick="schedule_btn('${h.hotel_code}')"
										value="${h.hotel_name}">
								</c:forEach>
						...
						<div class="landmark_des">
							<div class="hi_top">
								<c:forEach var="l" items="${llist}">
									<input type="button" class="btn_class"
										onclick="schedule_btn2('${l.land_code}')"
										value="${l.land_name}">
								...
								<c:forEach var="l" items="${llist}">
									...
									<div class="ld_detail_bottom">
										<div class="ld_bottom_middle" id="${l.land_code}">
											<br> <img src="${l.land_pic01 }" width="27%"
												height="200" style="border-radius: 5px; margin: 15px;" /> <img
				...
				<div class="main_schedule" style="height: 700px;" id="div4">
					...
					<ul id="information">
						...
						<br> ▶ 북마리아나 관광청 사이트
						http://www.mymarianas.co.kr/html/main/main.asp (레스토랑, 쇼핑, 관광지등 에
						대한 정보들을 얻으실 수 있습니다.)
						<br>▶ 해외 안전 여행사이트 https://www.0404.go.kr/dev/main.mofa
						(외교통상부)
						...
						<br>본 상품은 고객 결제 후 해피콜 시 여행출발에 대한 최종 확정 여부를 결정하며, 출발 확정 이후 취소
						시 귀책사유에 따라 다음의 약관에 따릅니다.
			...
			<div id="reservation_form">
				...
				<div class="rf">
					2. 항공 선택 <br> <select class="select_option" name="res_airport">
				...
				<div class="rf">
					...
					<c:if test="${!empty hlist}">
						...
						<select class="select_option" id="res_hotel" name="res_hotel">
							<c:forEach var="h" items="${hlist}">
					...
					<table
						...
						<tr>
							...
							<td><input readonly type="text" id="res_total_price"
								name="res_total_price" value="0"
								style="border: none; background-color: #F6F6F6; font-size: 26px; color: red; text-align: right; width: 150px; border-color: #F6F6F6; pointer-events: none;" /></td>
							<td style="font-size: 16px;">원</td>
						</tr>
					</table>
				</div>
				<button type="submit" id="btn_reservation">예약하기</button>
